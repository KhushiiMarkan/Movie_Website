document.addEventListener('DOMContentLoaded', () => {
    ...
    let isPlaying = false;
    const videoId = "iP-l9zQEL3U"; // Updated Video ID for the Munjya Movie trailer
    const originalSrc = `https://www.youtube.com/embed/${videoId}`; // Updated video URL without autoplay or mute
...
timePoints.forEach(timePoint => {
    ...
    // Function to navigate to the next page and pass seat and date data
    arrowBtn.addEventListener('click', () => {
        if (selectedSeats.length > 0 && selectedDate && selectedTime) {
            const totalPrice = selectedSeats.length * seatPrice;

            // Store selected seats, total price, and selected date in localStorage
            ...
            window.location.href = 'khel khel mei display.html';
        } else {
            alert('Please select at least one seat, a date and time.');
        }
    });
});
